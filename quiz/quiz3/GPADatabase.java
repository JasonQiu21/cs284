...
class HeapNode<S extends Comparable<S>, T extends Comparable<T>>{
    ...
    public int compareHeapNode(S new_value1, T new_value2) {
		if(value2.compareTo(new_value2) > 0){return 1;}
		if(value2.compareTo(new_value2) == 0){return 0;}
    	return -1;
    }
...
{    
    ...
    public int compareAVLNode(S new_value1, T new_value2) {
		if(value1.compareTo(new_value1) > 0){return 1;}
		if(value1.compareTo(new_value1) == 0){return 0;}
    	return -1;    }
...
public class GPADatabase {
	...
	public void insertStudent(String student_name, Double gpa) {
		this_heap.add(new HeapNode(student_name, gpa));
	 ...
	 */
	public ArrayList<String> helper(int k, ArrayList<String> names){
		if (k <= 0){return names;}
		else{
			HeapNode<String, Double> removed;
			try{
				removed = this_heap.poll();
			} catch(NoSuchElementException e){
				return names;
			}
			this_avltree.deleteNode(removed.value1, removed.value2);
			names.add(removed.value1);
			return helper(k-1, names);
		}
	}
	public ArrayList<String> removeTopkStudent(int k) {
		return helper(k, new ArrayList<String>());
	}
	...
	public Double searchStudentGPA(String student_name) {
		AVLNode<String, Double> gpa = this_avltree.search(student_name, 0.0);
		if(gpa == null){return null;}
		return (Double) gpa.value2;
	}
